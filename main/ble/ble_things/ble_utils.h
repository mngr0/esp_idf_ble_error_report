...
static uint16_t inline generate_uuid(uint16_t base, uint16_t index) {
  //ESP_LOGI("BLE_UTILS", "BASE:%04x \tINDEX:%04x(%d) \tRESULT:%04x",base, index,index,(base & 0xff00) | (index & 0x00ff));
  return (base & 0xff00) | (index & 0x00ff);
}

void allocate_conf_dynamic(uint16_t conf_len, uint8_t srvc_inst_id,
                           uint16_t *uuid_ptr);
void allocate_status_dynamic(uint16_t status_len, uint8_t srvc_inst_id,
                             uint16_t *uuid_ptr);
...
void allocare_una_caratteristica_status(esp_gatts_attr_db_t *input,
                                        uint16_t base, uint8_t *GATT_CHAR_UUID);

void allocare_una_caratteristica_main(esp_gatts_attr_db_t *input,
                                        uint16_t base, uint8_t *GATT_CHAR_UUID);
