...
static const uint16_t GATT_BLUEFI_UUID_SERVICE = (0xFF00 | GATT_BLUEFI_IDX_SERVICE) & (PROFILE_BLUEFI <<8);
...
const esp_gatts_attr_db_t gatt_bluefi_db[HRS_IDX_NB] =
    {
...
void bluefi_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
    {
        ...
        if (set_dev_name_ret)
        {
            ESP_LOGE(GATTS_BLUEFI_TAG, "set device name failed, error code = %x", set_dev_name_ret);
        }
        esp_err_t raw_adv_ret = esp_ble_gap_config_adv_data_raw(raw_adv_data, sizeof(raw_adv_data));
        if (raw_adv_ret)
        {
            ESP_LOGE(GATTS_BLUEFI_TAG, "config raw adv data failed, error code = %x ", raw_adv_ret);
        }
        gatt_ar_state_adv_config_done |= ADV_CONFIG_FLAG;
        esp_err_t raw_scan_ret = esp_ble_gap_config_scan_rsp_data_raw(raw_scan_rsp_data, sizeof(raw_scan_rsp_data));
        if (raw_scan_ret)
        {
            ESP_LOGE(GATTS_BLUEFI_TAG, "config raw scan rsp data failed, error code = %x", raw_scan_ret);
        }
        gatt_ar_state_adv_config_done |= SCAN_RSP_CONFIG_FLAG;

        esp_err_t create_attr_ret = esp_ble_gatts_create_service(gatts_if, &heart_rate_profile_tab[PROFILE_BLUEFI].service_id, GATT_BLUEFI_NB);
        if (create_attr_ret)
        {
            ESP_LOGE(GATTS_BLUEFI_TAG, "create attr table failed, error code = %x", create_attr_ret);
        }
    }
    break;
