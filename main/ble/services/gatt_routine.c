...
static const uint16_t GATT_HANDLE_STATUS_VALUE =
    (GATT_HANDLE_IDX_STATUS_VALUE) | (PROFILE_ROUTINE_IDX << 4);
static const uint16_t GATT_HANDLE_CONFIG_VALUE =
    (GATT_HANDLE_IDX_CONFIG_VALUE) | (PROFILE_ROUTINE_IDX << 4);
static const uint16_t GATT_HANDLE_COMMAND_VALUE =
    (GATT_HANDLE_IDX_COMMAND_VALUE) | (PROFILE_ROUTINE_IDX << 4);
...
const esp_gatts_attr_db_t gatt_routine_db[GATT_HANDLE_NB] = {
    ...
    [GATT_HANDLE_IDX_CONFIG_VALUE] = {{ESP_GATT_AUTO_RSP},
                                      {ESP_UUID_LEN_16,
                                       ...
                                       GATTS_DEMO_CHAR_VAL_LEN_MAX, 1024,
                                       (uint8_t *)routine_descriptor.config}},
...
bool gatt_routine_send_logger_update_to_client(char *json_status) {
  return gatt_handle_send_logger_update_to_client(&routine_descriptor,
                                                  json_status);
}
