...
void ar_conf_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
        ESP_LOGI(GATTS_AR_CONF_TAG, "ESP_GATTS_WRITE_EVT");
        break;
    ...
    case ESP_GATTS_CREAT_ATTR_TAB_EVT:
     ESP_LOGI(GATTS_AR_CONF_TAG, "creat attr tab, MTU %d", param->mtu.mtu);
        break;
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(gatt_ar_conf_db, gatts_if, GATT_AR_CONF_NB, SVC_INST_ID);
        if (create_attr_ret)
        {
...
#define GATTS_TAG GATTS_AR_CONF_TAG
#define gl_profile_tab heart_rate_profile_tab

static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
    case ESP_GATTS_REG_EVT:
        ESP_LOGI(GATTS_TAG, "REGISTER_APP_EVT, status %d, app_id %d\n", param->reg.status, param->reg.app_id);
        gl_profile_tab[PROFILE_AR_CONF_IDX].service_id.is_primary = true;
        gl_profile_tab[PROFILE_AR_CONF_IDX].service_id.id.inst_id = 0x00;
        gl_profile_tab[PROFILE_AR_CONF_IDX].service_id.id.uuid.len = ESP_UUID_LEN_16;
        gl_profile_tab[PROFILE_AR_CONF_IDX].service_id.id.uuid.uuid.uuid16 = GATTS_SERVICE_UUID_TEST_B;

        esp_ble_gatts_create_service(gatts_if, &gl_profile_tab[PROFILE_AR_CONF_IDX].service_id, GATTS_NUM_HANDLE_TEST_B);
        break;
